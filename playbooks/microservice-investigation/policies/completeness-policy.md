...
| Tier | Marker | Rule |
|---|---|---|
| **Minimum Viable** | `[REQUIRED]` | MUST be filled before the phase can be considered complete. If the information cannot be obtained, the field MUST contain `[unknown]` with a brief explanation of why. |
...
| Notable patterns and conventions | `[STANDARD]` | DI, error handling, logging patterns |
...
1. All `[REQUIRED]` fields in the phase's output templates are filled (or explicitly marked `[unknown]` with justification).
